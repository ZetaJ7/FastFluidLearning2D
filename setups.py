...
artery1 = (torch.mean(torch.tensor(np.asarray(Image.open('imgs/artery1.png'))).float(),dim=2)<100).float()
artery2 = (torch.mean(torch.tensor(np.asarray(Image.open('imgs/artery2.png'))).float(),dim=2)<100).float()
obstacle1=(torch.mean(torch.tensor(np.asarray(Image.open('imgs/obstacle1.png'))).float(),dim=2)<100).float()
obstacle2=(torch.mean(torch.tensor(np.asarray(Image.open('imgs/obstacle2.png'))).float(),dim=2)<100).float()
obstacle3=(torch.mean(torch.tensor(np.asarray(Image.open('imgs/obstacle3.png'))).float(),dim=2)<100).float()

#images = {"blank":blank,"cyber":cyber_truck, "fish":fish, "smiley":smiley, "wing":wing}
images = {"obs1":obstacle1,"obs2":obstacle2, "obs3":obstacle3}
#backgrounds = {"empty":background1,"cave1":background2,"cave2":background3}
#backgrounds = {"empty":background1,"artery1":artery1,"artery2":artery2}
backgrounds = {"artery1":artery1,"artery2":artery2}
"""
ask-tell interface:
...
class Dataset:
	def __init__(self,w,h,batch_size=100,dataset_size=1000,average_sequence_length=5000,interactive=False,max_speed=3,brown_damping=0.9995,brown_velocity=0.005,init_velocity=0,init_rho=None,n_cond=False,dt=1,types=["magnus","box","pipe"],images=["blank","cyber","fish","smiley","wing"],background_images=["empty"]):
		"""
		create dataset
		:w: width of domains
		:h: height of domains
